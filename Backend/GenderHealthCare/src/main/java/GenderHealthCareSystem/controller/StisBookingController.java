...
public class StisBookingController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponse<List<StisBooking>>> getAllBookings() {
        List<StisBooking> bookings = stisBookingService.getAllBookings();
        return ResponseEntity.ok(new ApiResponse<>(HttpStatus.OK, "Fetched all bookings", bookings, null));
    }

    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<StisBooking>> getBookingById(@PathVariable Integer id) {
        Optional<StisBooking> booking = stisBookingService.getBookingById(id);
    ...
    @GetMapping("/history")
    public ResponseEntity<ApiResponse<List<StisBooking>>> getBookingHistoryByCustomer(@AuthenticationPrincipal Jwt jwt) {
        // Fetch booking history for a specific customer
        List<StisBooking> bookingHistory = stisBookingService.getBookingHistoryByCustomer(Integer.parseInt(jwt.getClaimAsString("userID")));
        return ResponseEntity.ok(new ApiResponse<>(HttpStatus.OK, "Fetched booking history", bookingHistory, null));
    }

    @PostMapping
    public ResponseEntity<ApiResponse<StisBooking>> createBooking(@RequestBody StisBooking booking) {
